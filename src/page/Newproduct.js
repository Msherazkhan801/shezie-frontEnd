...
const Newproduct = () => {
  ...
  const handleSubmit = async(e)=>{
    ...
    if(name && image && category && price){
      const fetchData = await fetch(`${envFile}/uploadProduct`,{
        method : "POST",
        headers : {
      ...
      console.log(process.env.REACT_APP_SERVER_DOMIN,"ENV");
  ...
  return (
    <div className="p-4">
       <form className='m-auto w-full max-w-md  shadow flex flex-col p-3 bg-white' onSubmit={handleSubmit}>
        ...
        <select className='bg-slate-200 p-1 my-1 w-full' id='category' name='category' onChange={handleOnChange} value={data.category}>
        ...
        <div  className='h-40 w-full bg-slate-200  rounded flex items-center justify-center cursor-pointer'>
            {
              data.image ? <img src={data.image} className="h-full" alt='pic'/> :<span className='text-5xl'><BsCloudUpload/></span> 
            }
